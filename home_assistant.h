...
class HomeAssistant {
  public:
    ...
    HomeAssistant(String _devName, int _tID, String homeAssistantFriendlyName,
                  String project = "", String version = __HA_VERSION__,
    ...
    void addSensor(/*String devName, String HAname, */ String topic_sub_name,
                   String friendlyName, String unitDesc, String className,
                   String iconName) {
    ...
    void publishAttrib(String attrTopic) {
        if (macAddress == "")
            macAddress = WiFi.macAddress();
        String attrib = "{\"Rssi\":" + String(rssiVal) + "," + "\"Mac\": \"" +
                        macAddress + "\"," + "\"IP\": \"" + ipAddress + "\"," +
                        "\"Host\": \"" + hostName + "\"," + "\"Version\": \"" +
                        swVersion + "\"," + "\"Manufacturer\": \"muWerk\"";
        if (muProject != "") {
    ...
    void mqMsg(String topic, String msg, String originator) {
        if (topic == "net/rssi") {
            ...
            if (useHA) {
                ...
                String HAcmd = "";
                char *p0 = strchr(
                    cmd, '/');  // get hostname from mqtt message, e.g. myhost
                if (p0) {
                    ++p0;
                    HAcmd = String(p0);
                }
                if (!strcmp(cmsg, "connected")) {
                    ...
                    for (unsigned int i = 0;
                         i < sensor_topic_sub_names.length(); i++) {
                        String subDevNo = String(i + 1);
                        String HAstateTopic = HAmuPrefix + "/" + devName +
                                              "/sensor/" +
                                              sensor_topic_sub_names[i];
                        String HAattrTopic = devName + "/sensor/" +
                                             sensor_topic_sub_names[i] +
                                             "/attribs";
                        String HAdiscoTopic = "!" + HAprefix + "/sensor/" +
                                              subDevNo + "-" + HAnameNS + "/" +
                                              devName + "/config";
                        String HAdiscoEntityDef =
                            "{\"stat_t\":\"" + HAstateTopic + "\"," +
                            "\"json_attr_t\":\"" + HAmuPrefix + "/" +
                            HAattrTopic + "\"," + "\"name\":\"" + HAname + " " +
                            sensor_friendlyNames[i] + "\"," + "\"uniq_id\":\"" +
                            macAddress + "-" + devName + "-S" + subDevNo +
                            "\"," + "\"val_tpl\":\"{{ value | float }}\"," +
                            "\"unit_of_meas\":\"" + sensor_unitDescs[i] +
                            "\"," + "\"expire_after\": 1800," + "\"icon\":\"" +
                            sensor_iconNames[i] + "\"";
                        if (sensor_classNames[i] != "None") {
                            HAdiscoEntityDef += ",\"device_class\":\"" +
                                                sensor_classNames[i] + "\"";
                        }
                        HAdiscoEntityDef =
                            HAdiscoEntityDef + ",\"device\":{" +
                            "\"identifiers\":[\"" + macAddress + "-" + devName +
                            "\",\"" + macAddress + "-" + devName + "-S" +
                            subDevNo + "\"]," + "\"model\":\"" + muProject +
                            "\"," + "\"name\":\"" + capHostName + "\"," +
                            "\"manufacturer\":\"muWerk\"," +
                    ...
                    for (int i = 0; i < nrLights; i++) {
                        ...
                        String HAcommandTopic =
                            HAcmd + "/" + devName + "/light/set";
                        String HAstateTopic =
                            HAmuPrefix + "/" + devName + "/light/state";
                        String HAattrTopic = devName + "/light/attribs";
                        String HAcommandBrTopic =
                            HAcmd + "/" + devName + "/light/set";
                        String HAstateBrTopic = HAmuPrefix + "/" + devName +
                                                "/light/unitbrightness";
                        String HAdiscoTopic = "!" + HAprefix + "/light/" +
                                              subDevNo + "-" + HAnameNS + "/" +
                                              devName + "/config";
                        String HAdiscoEntityDef =
                            "{\"stat_t\":\"" + HAstateTopic + "\"," +
                            "\"name\":\"" + HAname + "\"," + "\"uniq_id\":\"" +
                            macAddress + "-" + devName + "-L" + subDevNo +
                            "\"," + "\"cmd_t\":\"" + HAcommandTopic + "\"," +
                            "\"json_attr_t\":\"" + HAmuPrefix + "/" +
                            HAattrTopic + "\"," + "\"bri_stat_t\":\"" +
                            HAstateBrTopic + "\"," + "\"bri_scl\":\"100\"," +
                    ...
                    for (int i = 0; i < nrSwitches; i++) {
                        ...
                        String subDevNo = String(id);
                        String HAcommandTopic =
                            HAcmd + "/" + devName + "/switch/set";
                        String HAstateTopic =
                            HAmuPrefix + "/" + devName + "/switch/state";
                        String HAattrTopic = devName + "/switch/attribs";
                        String HAdiscoTopic = "!" + HAprefix + "/switch/" +
                                              subDevNo + "-" + HAnameNS + "/" +
                                              devName + "/config";
                        String HAdiscoEntityDef =
                            "{\"stat_t\":\"" + HAstateTopic + "\"," +
                            "\"name\":\"" + HAname + "\"," + "\"uniq_id\":\"" +
                            macAddress + "-" + devName + "-SW" + subDevNo +
                            "\"," + "\"cmd_t\":\"" + HAcommandTopic + "\"," +
                            "\"json_attr_t\":\"" + HAmuPrefix + "/" +
                            HAattrTopic + "\"," + "\"state_on\":\"on\"," +
                            "\"state_off\":\"off\"," + "\"pl_on\":\"on\"," +
                            "\"pl_off\":\"off\"," + "\"icon\":\"" +
                            switch_iconNames[i] + "\"";
                        HAdiscoEntityDef =
                            HAdiscoEntityDef + ",\"device\":{" +
